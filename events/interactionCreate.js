...
module.exports = async (client) => {
    client.on('interactionCreate', async interaction => {
        ...
        if (interaction.customId === 'create_ticket') {
            // Retrieve the stored category ID from the database
            let ticketDb = {};
            if (fs.existsSync(ticketDbPath)) {
                ticketDb = JSON.parse(fs.readFileSync(ticketDbPath, 'utf8'));
            }

            // Debug: Log what is being read from the file
            console.log(`Retrieved data for guild ${interaction.guild.id}:`, ticketDb[interaction.guild.id]);

            const categoryData = ticketDb[interaction.guild.id];
            if (!categoryData) {
                return interaction.reply({ 
                    content: 'Ticket category not found! Please set it up with ?panelsetup.', 
                    ephemeral: true 
                });
            }

            const category = interaction.guild.channels.cache.get(categoryData.categoryId);
            if (!category) {
                return interaction.reply({ 
                    content: 'The ticket category is invalid or deleted.', 
                    ephemeral: true 
                });
            }
