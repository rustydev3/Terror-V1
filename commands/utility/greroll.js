...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (
      ...
      return message.channel.send({
        embeds: [
          new MessageEmbed()
            .setColor(`#2f3136`)
            .setDescription(`${client.emoji.cross} | You need **Manage Server** permission or the 'Giveaway Manager' role to reroll a giveaway.`)
        ]
      });
    }
    ...
    if (validEntries.size === 0) {
      return message.channel.send({
        embeds: [
          new MessageEmbed()
            .setColor(`#2f3136`)
            .setDescription(`${client.emoji.cross} | No valid entries, cannot reroll the giveaway.`)
        ]
      });
    }
    ...
    message.channel.send(
      `🎉 Congratulations <@${newWinner.id}>! You are the new winner of the ${prize}** giveaway!`
    );
  }
};